...
class CanvasText(CTkBaseClass):
    def __init__(self,window,parent, text, location,corrector,  values, taula, croquis_en_us, dataframe, interval = None, sheet = None ,orientation= 0,color = "White", text_size = 9, list_len = 10):
        ...
        self.span = (self.corrector[0][0]-self.corrector[0][1], self.corrector[1][0]-self.corrector[1][1])  #UNUSED, Dimensions de la figura
        ...
        self.lb = tkinter.Listbox(self.parent, width=12 ,background="#2B2B2B", fg="#FFFFFF", bd= 0, relief="flat", activestyle="none", highlightthickness=0, justify="left" )
    ...
    def on_click(self, _event):
        ...
        if not self.toggle:
            ...
            self.lb.place(x=self.parent.coords(self.txt)[0]- self.entry_width/2+2,
                          y=self.parent.coords(self.txt)[1]+ self.entry_height-5)
